<?php
    ...
    class sample extends config\environment {
        ...
        public function __construct() {
            ...
            // Core
            $this->merge(new core\config([
                'site_name' => 'Sample Site',
            ]));

            // HTTP
            $this->merge(new http\config([
            ...
            $this->merge(new assets\processor\css\config([
                'search_replace' => [
                    '//dev.env.example.com/' => '//example.com/',
                ],
            ]));

            // Locale
            $this->merge(new locale\config([
            ...
            // Web
            $this->merge(new web\config([
                'user_agent' => 'php-neoform-crawler/1.0',
            ]));

            // SQL
            $this->merge(new sql\config([
            ...
            // Auth
            $this->merge(new auth\config);

            // Memcache
            $this->merge(new memcache\config([
            ...
            // APC
            $this->merge(new apc\config([
                // Prefix key
                'key_prefix' => 'sample',
            ]));

            // Email
            $this->merge(new email\config([
