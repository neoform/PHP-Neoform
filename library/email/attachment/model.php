<?php
    ...
    class email_attachment_model {
        ...
        public function send($email, $name = "") {
            ...
            // error check
            if (! is_email($email)) {
                throw new exception_mail('Email address "' . $email . '" is not valid');
            }

            if ($this->text_body == "" && $this->html_body == "") {
                throw new exception_mail("No email body (text or html) was set.");
            }
            ...
            if (mail($email, $this->subject, $this->body, $this->headers)) {
                ...
                throw new exception_mail("PHP mail function failed for unknown reason");
            }
        }
